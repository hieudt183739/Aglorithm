...
void main()
{
    Init_System();
    LCD_init();
    LCD_Write_String("Xin chao KTMT61");// muốn ghi gì thì viết thế này
...
void LCD_Send_Command(unsigned char x)
{
    ...
    RW = 0;      //De ghi du lieu
    EN = 1;      // phải bật lên 1 rồi tắt về 0 thì LCD mới hoạt động
    Delay_ms(1); // cho dể LCD thực hiện lệnh
    EN = 0;
